...
const PostCard = ({ post }: { post: Post }) => {
  ...
  return (
    <div className="group flex flex-col max-w-sm bg-gray-100 border border-gray-200 rounded-lg shadow dark:bg-dark-400 dark:border-gray-700 overflow-hidden">
      ...
      <div className="w-full p-3 flex flex-col items-start gap-2">
        <Link href={`/search?category=${category}`} className="">
          <button className="border border-gray-300 rounded-full text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-dark-300 hover:bg-gray-200 dark:hover:bg-dark-500 font-medium text-[12px] tracking-wider py-1 px-3">
            {post && post.category}
          </button>
        </Link>
        ...
        <div className="flex justify-between items-center py-1 w-full group-hover:px-[4px] duration-200">
          ...
          <Link
            href={`/post/${slug}`}
            className="flex gap-2 items-center text-primary-500 dark:text-primary-100   hover:bg-gray-200 dark:hover:bg-dark-500 duration-150 rounded-lg py-1 px-2 hover:pl-2"
          >
